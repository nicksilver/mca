...
syms = ['*_pr_perc_*']
tbeg = ['Percent Change in Monthly Precipitation (%) ']
...
for i, sym in enumerate(syms):
    ...
    for f in fnames:
        ...
        zs = ms.zstats(gis_path+shp_name, ens_mean, precip=True, units='metric')
        zs_range = ms.zstats_range(mod_delta, gis_path+shp_name, zs, mod_list,
                                   precip=True, units='metric')
        ...
        mplt.clim_div_grid(zs, stat='median', title=title, browser=True,
                           r_data=zs_range, var='precip', save_path=save_path+fname)
